...
export function Dashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      const { data: sData } = await supabase.from('sales').select('total_amount');

      const total = sData?.reduce((acc, sale) => acc + Number(sale.total_amount), 0) || 0;
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-green-500">
          <p className="text-sm font-bold text-gray-500 uppercase">Faturamento Total</p>
          <div className="flex items-center justify-between mt-2">
            <h2 className="text-2xl font-bold">{isAdmin ? `R$ ${stats.sales.toFixed(2)}` : 'R$ *****'}</h2>
